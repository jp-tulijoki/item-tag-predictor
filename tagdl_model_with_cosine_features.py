...
def predict(data):
    ...
    with torch.no_grad():
        ...
        output_df.to_csv(f"predictions/{prediction_folder}/predictions{TENFOLD}.csv", index=False)
...
def create_prediction_folder():
    if not os.path.exists(f"predictions/{prediction_folder}"):
        os.makedirs(f"predictions/{prediction_folder}")
...
for i, combination in enumerate(COMBINATIONS):
    ...
    if os.path.exists(f"predictions/{prediction_folder}") and OVERWRITE == False:
    ...
    for TENFOLD in range(10):
        ...
        model = FFNN(1102 + len(selected_features), hidden_size = 64)
        model.to(device)
