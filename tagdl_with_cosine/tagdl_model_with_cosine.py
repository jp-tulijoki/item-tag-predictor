...
LEARNING_RATE = 1e-04
TENFOLD = 0
COSINE_SIM_FILE = "cosine.csv"
PREDICTION_FILE = f"predictions{TENFOLD}_cosine.csv"
...
def train(data, epoch):
    ...
    for i in range(int(data.shape[0]/BATCH_SIZE + 1)):
        ...
        input = torch.cat((torch.tensor(batch[["log_IMDB", "log_IMDB_nostem", "rating_similarity", "avg_rating", "tag_exists", "lsi_tags_75", "lsi_imdb_175", "tag_prob", "cosine"]].values.tolist()), torch.tensor(batch["one_hot"].values.tolist())), dim=1).to(device, dtype=torch.float)
...
cosine = pd.read_csv(COSINE_SIM_FILE)
SCALING_MIN = -0.7506188543104041 #negative value for tag exists
SCALING_MAX = 1.33220801542758 #positive value for tag exists
cosine["cosine"] = (cosine["cosine"] - cosine["cosine"].min()) / (cosine["cosine"].max() - cosine["cosine"].min()) * (SCALING_MAX - SCALING_MIN) + SCALING_MIN
...
train_data = pd.merge(train_data, cosine, on=["item_id", "tag"], how="left")
test_data = pd.merge(test_data, cosine, on=["item_id", "tag"], how="left")
...
model = FFNN(1103, hidden_size = 64)
model.to(device)
