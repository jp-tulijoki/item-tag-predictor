...
def train(data, epoch):
    ...
    for i in range(int(data.shape[0]/BATCH_SIZE)):
        ...
        input = torch.cat((torch.tensor(batch["embedding"].map(loads).tolist()), torch.tensor(batch[["log_IMDB", "log_IMDB_nostem", "rating_similarity", "avg_rating", "tag_exists", "lsi_tags_75", "lsi_imdb_175", "tag_prob"]].values.tolist()), torch.tensor(batch["one_hot"].values.tolist())), dim=1).to(device, dtype=torch.float)
...
def predict(data):
    ...
    with torch.no_grad():
        ...
        output_df.to_csv(f'predictions{TENFOLD}/predictions{TENFOLD}.csv', index=False)
...
test_data = pd.merge(embeddings, test_file, on="item_id", how="left").dropna()
test_data["one_hot"] = test_data["tag"].apply(lambda t: get_one_hot(t))

print(train_data.shape)
print(test_data.shape)

model = FFNN(1870, hidden_size=64) 
model.to(device)
